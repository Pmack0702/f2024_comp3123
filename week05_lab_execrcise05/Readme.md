...
- This helps developers to create a sub-application within the main application, which help to organize routes into separate modules.  - It’s like splitting a big job into smaller tasks that are easier to manage.
...
- When things go wrong in an app (like when a file is missing or there’s a server issue), you want to handle those errors properly so the app doesn’t crash and users see a helpful message. Express lets you do this with error-handling functions.

- For example:
- If something goes wrong in a route, you can send the error to a special function that handles errors for you.
- This function can decide what message to show the user (like "File not found") and log the error for debugging.

- This helps users to show a blank page or letting the app crash, you can show a clear message explaining the issue.
